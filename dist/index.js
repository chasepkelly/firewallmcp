...
class AppraisalFirewallMCPServer {
    ...
    async createResidentialAppraisalOrder(args) {
        this.validateConfig();
        const payload = {
    ...
    async createCommercialAppraisalOrder(args) {
        this.validateConfig();
        const payload = {
            apikey: this.config.apiKey,
            apisecret: this.config.apiSecret,
    ...
    validateConfig() {
        if (!this.config.baseUrl || !this.config.apiKey || !this.config.apiSecret) {
            throw new McpError(ErrorCode.InvalidRequest, 'Missing required configuration. Please set APPRAISAL_FIREWALL_BASE_URL, APPRAISAL_FIREWALL_API_KEY, and APPRAISAL_FIREWALL_API_SECRET environment variables.');
        }
    }
    async run() {
